...
// `text-cerneala-3` se recalculează pe suprafața de deasupra, deci același rând stă corect
...
export default function MecanismRandFisa({ titlu, children }: Props) {
  return (
    <div className="grid gap-3 border-t py-8 last:border-b lg:grid-cols-[20rem_minmax(0,1fr)] lg:gap-8 lg:py-10">
      <h3 className="font-afis max-w-[24ch] text-[clamp(1.15rem,1.9vw,1.5rem)] font-semibold  text-cerneala">
        {titlu}
      </h3>
      <p className="max-w-[64ch] text-[17px] leading-[1.6] text-cerneala-3">{children}</p>
    </div>
  );
}
